...
public class FormateurDaoImp implements FormateurDao{
    ...
    private static final String FIND_ALL_STUDENTS = "select `nom`, `prenom`, `cin` from user,apprenant,specialite where user.id_user=apprenant.id_user and apprenant.id_sp=specialite.id_specialite and nom_sp=?";
    private static final String FIND_A_STUDENT = "SELECT * FROM `absence` WHERE id_appr = ?";
    private static final String SHOW_STUDENT = "SELECT `id_absence`, `prenom`, `nom`, `dateAbsence`, `absences`, `cin` FROM user,apprenant,absence where user.id_user=apprenant.id_user AND absence.id_appr=apprenant.cin AND absence.dateAbsence=?";
    private static final String INSERT_AN_ABSENCE = "INSERT INTO `absence` (absences,dateAbsence, id_appr) VALUES (?, ?, ?)";
    private static final String UPDATE_AN_ABSENCE = "UPDATE `absence` SET absences=? WHERE id_appr=?";
    private static final String DELETE_AN_ABSENCE = "DELETE FROM `absence` WHERE id_absence = ? AND dateAbsence = CURRENT_DATE";
...
//    }
    ...
    public ObservableList<AbUser> AfficheInfos(String dateArg) throws SQLException, ClassNotFoundException {
        ...
        while (rs.next()) {
            apprenantAbsente = new AbUser(rs.getInt("id_absence"), rs.getString("nom"), rs.getString("prenom"), rs.getString("dateAbsence"), rs.getString("absences"),rs.getString("cin"));
